/*
Given a sorted positive integer array nums and an integer n, add/patch elements to the array such that 
any number in range [1, n] inclusive can be formed by the sum of some elements in the array. Return 
the minimum number of patches required.

Example 1:
nums = [1, 3], n = 6
Return 1.

Combinations of nums are [1], [3], [1,3], which form possible sums of: 1, 3, 4.
Now if we add/patch 2 to nums, the combinations are: [1], [2], [3], [1,3], [2,3], [1,2,3].
Possible sums are 1, 2, 3, 4, 5, 6, which now covers the range [1, 6].
So we only need 1 patch.

Example 2:
nums = [1, 5, 10], n = 20
Return 2.
The two patches can be [2, 4].

Example 3:
nums = [1, 2, 2], n = 5
Return 0.
*/

import java.util.Hashtable;
import java.util.HashMap;
import java.util.Map; //use hashmap or hashtable

public class PatchingArray {
	public static int minPatches(int[] nums, int n) {
        if( n <= 0 || nums.length<0) return -1;
        int added = 0, i=0;
        long miss = 1; // see 2nd example
        while(miss <= n){
            if(i < nums.length && miss >= nums[i]){
                miss += nums[i];
                i++;
            }else{
                miss += miss;
                added++;
            }
        }
        return added;
    }

	public static void main(String[] args) {
		int[] nums1 = new int[] {1, 3};
		int n = 6;
		int res = minPatches(nums1, n);
		for(int i=0; i < nums1.length; i++){
			System.out.println(nums1[i] + " ");
		}
        System.out.println("Patches for " + n + ": " + res);
		int[] nums2 = new int[] {1,2,31,33};
		n = 2147483647;
		res = minPatches(nums2, n);
		for(int i=0; i < nums2.length; i++){
			System.out.println(nums2[i] + " ");
		}
        System.out.println("Patches for " + n + ": " + res);
		return;
	}	
}
